...
class FlutterInappPurchase {
  ...
  Future<bool> openSubscriptionsStore({bool asAnotherTask = false}) async {
    if (_platform.isAndroid) {
      return (await _channel.invokeMethod<bool?>('openPlayStoreSubscriptions')) ?? false;
    } else if (_platform.isIOS) {
      await IosOpenSubscriptionsSettings.openSubscriptionsSettings(asAnotherTask: asAnotherTask);
      return true;
    }
    throw PlatformException(code: _platform.operatingSystem, message: "platform not supported");
  }
  ...
  Future requestSubscription(
    ...
    if (_platform.isAndroid) {
      return await _channel.invokeMethod('buyItemByType', <String, dynamic>{
        ...
        'obfuscatedAccountId': obfuscatedAccountId ?? userUID,
        'obfuscatedProfileId': obfuscatedProfileIdAndroid ?? userUID,
        'purchaseToken': purchaseTokenAndroid,
        'offerTokenIndex': offerTokenIndex,
      });
    } else if (_platform.isIOS) {
  ...
  Future<http.Response> validateReceiptIos({
    ...
    final String url =
        isTest ? 'https://sandbox.itunes.apple.com/verifyReceipt' : 'https://buy.itunes.apple.com/verifyReceipt';
